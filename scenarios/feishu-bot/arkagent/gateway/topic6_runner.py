...
from .pipeline_store import HC_KINDS, PipelineJob, PipelineStore
...
class Topic6Runner:
    ...
    async def _drive_session(self, job: PipelineJob, first_user_message: str) -> None:
        ...
        try:
            ...
            self._store.mark_failed(job_id, f"sse_stream_error: {error!s}"[:400])
            await self._reply_async(job.chat_id, f"topic6 执行异常:{error!s}"[:200])
        finally:
    ...
    async def _consume_events(self, job_id: str, stream) -> None:
        """SSE 事件循环:节流进度 → 累积 agent.message → 处理 idle 终态。"""
        ...
        async for event in stream:
            ...
            if etype == "agent.message":
                ...
                if body:
                    ...
                    self._maybe_update_phase(job_id, body)

            # 进度事件:节流后转发。
            progress = event_progress(event)
            if progress:
                now = time.monotonic()
                if progress != last_progress_text and now - last_progress_at >= PROGRESS_MIN_INTERVAL_SEC:
                    last_progress_at = now
                    last_progress_text = progress
                    await self._reply_async_by_job(job_id, f"⏳ {progress}")
            ...
            if etype in ("session.error", "session.status_failed"):
                ...
                await self._reply_async_by_job(job_id, f"❌ topic6 执行失败:{error[:200]}")
                return
    ...
    async def _handle_idle(self, job_id: str, collected: list[str]) -> None:
        ...
        summary = last[:300] if last else "topic6 pipeline 已完成。"
        await self._reply_async_by_job(job_id, f"✅ {summary}")

    # ---- helper --------------------------------------------------------------

    def _maybe_update_phase(self, job_id: str, body: str) -> None:
        match = re.search(r"\[phase\]\s+([A-Z0-9]+)", body)
        if match:
            self._store.update_phase(job_id, match.group(1))
